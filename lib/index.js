...
// Ningning image: package-relative first.
const IMAGE_CANDIDATES = [
  path.join(PACKAGE_ROOT, 'assets', 'ningning1.png'),
  path.join(PACKAGE_ROOT, 'assets', 'ningning2.png'),
]

// Size memory file: prefer writable DSH home locations.
const SIZE_FILE_CANDIDATES = [
  path.join(DSH_HOME, '.dshn-size.json'),
  path.join(DSH_HOME, 'profiles', 'web', '.dshn-size.json'),
]

// Usage ledger file (余额记账 mode): same policy as the size file.
const USAGE_FILE_CANDIDATES = [
  path.join(DSH_HOME, '.dshn-usage.json'),
  path.join(DSH_HOME, 'profiles', 'web', '.dshn-usage.json'),
]

// Sound assets: package-relative (ship Ya1/Ya2/D1/D2.mp3 in assets/).
const SOUND_SETS = {
  duck: {
...
if (window.__dshNingningWidget) return
window.__dshNingningWidget = true
...
var BALANCE_URL = '/dsh-ningning/balance.json'
var SIZE_URL = '/dsh-ningning/size.json'
var IMG_URL = '/dsh-ningning/image.png?v=2'
var GIF_URL = '/dsh-ningning/rua.gif'

var css = [
  '.dshnnv-root{position:fixed;right:0;bottom:0;--dshnn-scale:1;--dshnn-base:clamp(122px,calc(min(250px,min(100vw,100vh) * 0.28) * var(--dshnn-scale)),625px);width:var(--dshnn-base);height:var(--dshnn-base);pointer-events:none;user-select:none;-webkit-user-select:none;z-index:9999;font-family:inherit;transition:left .16s ease,top .16s ease,transform .3s ease}',
  '.dshnnv-root.dshnnv-left{transform:scaleX(-1)}',
  '.dshnnv-root.dshnnv-dragging{cursor:grabbing;transition:none}',
  '.dshnnv-body{position:absolute;left:0;top:0;width:100%;height:100%;transform-origin:50% 100%;transition:transform .22s cubic-bezier(.34,1.56,.64,1)}',
  '.dshnnv-img{position:absolute;right:0;bottom:0;width:59.45%;height:59.45%;display:block;pointer-events:none;-webkit-user-drag:none;user-select:none}',
  '.dshnnv-bubble{position:absolute;left:0;top:0;width:100%;aspect-ratio:1026/700;pointer-events:none;z-index:1;--dshnn-u:calc(var(--dshnn-base) / 1026)}',
  '.dshnnv-bubble svg{display:block;width:100%;height:100%;pointer-events:none}',
  '.dshnnv-bubble svg path,.dshnnv-bubble svg ellipse{pointer-events:none;cursor:pointer}',
  '.dshnnv-bubble.dshnnv-bubble-open svg path,.dshnnv-bubble.dshnnv-bubble-open svg ellipse{pointer-events:visiblePainted}',
  '.dshnnv-bubble .dshnnv-bshape,.dshnnv-bubble .dshnnv-b1,.dshnnv-bubble .dshnnv-b2{opacity:0;transform:scale(.7);transform-box:fill-box;transform-origin:50% 50%;transition:opacity .2s ease,transform .2s ease}',
  '.dshnnv-bubble.dshnnv-bubble-open .dshnnv-bshape,.dshnnv-bubble.dshnnv-bubble-open .dshnnv-b1,.dshnnv-bubble.dshnnv-bubble-open .dshnnv-b2{opacity:1;transform:none}',
  '.dshnnv-gif{position:absolute;left:44.25%;top:38%;transform:translate(-50%,-50%);max-width:calc(var(--dshnn-u) * 560);max-height:calc(var(--dshnn-u) * 400);display:none;opacity:0;transition:opacity .2s ease;pointer-events:none;-webkit-user-drag:none;user-select:none;object-fit:contain}',
  '.dshnnv-root.dshnnv-left .dshnnv-gif{transform:translate(-50%,-50%) scaleX(-1)}',
  '.dshnnv-bubble.dshnnv-bubble-open .dshnnv-gif{opacity:1}',
  '.dshnnv-bubble.dshnnv-bubble-open .dshnnv-b2{transition-delay:0s}',
  '.dshnnv-bubble.dshnnv-bubble-open .dshnnv-b1{transition-delay:.13s}',
  '.dshnnv-bubble.dshnnv-bubble-open .dshnnv-bshape{transition-delay:.26s}',
  '.dshnnv-bubble .dshnnv-bshape{transition-delay:.1s}',
  '.dshnnv-bubble .dshnnv-b1{transition-delay:.2s}',
  '.dshnnv-bubble .dshnnv-b2{transition-delay:.3s}',
  '.dshnnv-text{position:absolute;left:44.25%;top:38%;transform:translate(-50%,-50%);text-align:center;color:#536ba9;line-height:1.15;white-space:nowrap;pointer-events:none;opacity:0;transition:opacity .16s ease,transform .3s ease}',
  '.dshnnv-bubble.dshnnv-bubble-open .dshnnv-text{opacity:1;transition:opacity .16s ease .36s,transform .3s ease}',
  '.dshnnv-root.dshnnv-left .dshnnv-text{transform:translate(-50%,-50%) scaleX(-1)}',
  '.dshnnv-label{font-size:calc(var(--dshnn-u) * 66);font-weight:600;letter-spacing:.06em}',
  '.dshnnv-amount{font-size:calc(var(--dshnn-u) * 128);font-weight:800;line-height:1.05}',
  '.dshnnv-period{font-size:calc(var(--dshnn-u) * 104);font-weight:800;line-height:1.05}',
  '.dshnnv-wrap{white-space:normal;max-width:calc(var(--dshnn-u) * 560);line-height:1.2}',
  '.dshnnv-hint{font-size:calc(var(--dshnn-u) * 56);color:#9fb0d9;letter-spacing:.02em;margin-top:calc(var(--dshnn-u) * 9);min-height:calc(var(--dshnn-u) * 64);line-height:1.15}',
  '.dshnnv-menu-btn{position:absolute;top:calc(40.55% + 4px);right:4px;width:26px;height:26px;border:none;border-radius:6px;background:rgba(32,49,112,.85);cursor:pointer;pointer-events:auto;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;padding:0;z-index:2;opacity:0;transition:opacity .15s ease}',
  '.dshnnv-menu-btn.dshnnv-menu-btn-visible{opacity:1}',
  '.dshnnv-menu-btn span{display:block;width:14px;height:2px;background:#fff;border-radius:1px}',
  '.dshnnv-menu-btn:hover{background:#203170}',
  '.dshnnv-menu{position:fixed;min-width:196px;background:rgba(255,255,255,.92);border:1px solid rgba(32,49,112,.35);border-radius:10px;padding:10px 12px;opacity:0;transform:scale(.92) translateY(-4px);transform-origin:top right;transition:opacity .18s ease,transform .2s cubic-bezier(.34,1.56,.64,1);pointer-events:none;z-index:10000;box-shadow:0 6px 18px rgba(0,0,0,.18);color-scheme:light}',
  '.dshnnv-menu.dshnnv-menu-open{opacity:1;transform:scale(1) translateY(0);pointer-events:auto}',
  '.dshnnv-menu-row{display:flex;align-items:center;gap:8px;margin:5px 0;color:#203170;font-size:12px;white-space:nowrap}',
  '.dshnnv-range{flex:1;min-width:0;accent-color:#203170}',
  '.dshnnv-number{width:44px;border:1px solid rgba(32,49,112,.4);border-radius:6px;padding:2px 4px;font-size:12px;color:#203170;background:#fff;box-sizing:border-box}',
  '.dshnnv-number:disabled{opacity:.4;background:rgba(32,49,112,.06);cursor:not-allowed}',
  '.dshnnv-sound{flex:1;border:1px solid rgba(32,49,112,.4);border-radius:6px;background:rgba(32,49,112,.08);color:#203170;font-size:12px;padding:3px 0;cursor:pointer}',
  '.dshnnv-sound:hover{background:rgba(32,49,112,.16)}',
  '.dshnnv-check{width:16px;height:16px;accent-color:#203170;cursor:pointer;flex:0 0 auto}',
  '.dshnnv-menu-sep{height:1px;background:rgba(32,49,112,.25);margin:6px 0}',
  '.dshnnv-volpct{width:44px;text-align:right;color:#203170;font-size:12px}'
].join('\\n')
...
root.className = 'dshnnv-root'

var img = document.createElement('img')
img.className = 'dshnnv-img'
img.src = IMG_URL
img.alt = 'DeepSeek 余额'
img.draggable = false
...
menuBtn.type = 'button'
menuBtn.className = 'dshnnv-menu-btn'
menuBtn.title = '菜单'
menuBtn.innerHTML = '<span></span><span></span><span></span>'
menuBtn.addEventListener('click', function (e) { e.stopPropagation(); toggleMenu() })

var menuBox = document.createElement('div')
menuBox.className = 'dshnnv-menu'
...
scaleInput.step = '0.1'
scaleInput.className = 'dshnnv-range'
...
scaleNumber.max = '20'
scaleNumber.step = '1'
scaleNumber.className = 'dshnnv-number'
...
soundSelect.className = 'dshnnv-sound'
...
usageSelect.className = 'dshnnv-sound'
usageSelect.appendChild(soundOpt('ledger', '余额记账 (推荐)'))
...
peakSelect.className = 'dshnnv-sound'
...
bubbleToggle.className = 'dshnnv-check'
...
turnCostToggle.className = 'dshnnv-check'
...
turnCostCloseInput.className = 'dshnnv-number'
...
scrollGapToggle.className = 'dshnnv-check'
...
scrollGapInput.className = 'dshnnv-number'
...
volInput.max = '1'
volInput.step = '0.05'
volInput.className = 'dshnnv-range'
volInput.value = '0.9'
var volPct = document.createElement('span')
volPct.className = 'dshnnv-volpct'
...
menuSep1.className = 'dshnnv-menu-sep'
var row7 = menuRow()
...
textBox.className = 'dshnnv-text'
var labelEl = document.createElement('div')
labelEl.className = 'dshnnv-label'
labelEl.textContent = 'DeepSeek 余额'
var amountEl = document.createElement('div')
amountEl.className = 'dshnnv-amount'
var hintEl = document.createElement('div')
hintEl.className = 'dshnnv-hint'
textBox.appendChild(labelEl)
textBox.appendChild(amountEl)
textBox.appendChild(hintEl)

var bubbleBox = document.createElement('div')
bubbleBox.className = 'dshnnv-bubble'
bubbleBox.innerHTML = '<svg viewBox="0 0 1026 700" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg">' +
  '<path class="dshnnv-bshape" fill="#FFFFFF" stroke="#203170" stroke-width="18" stroke-linejoin="round" stroke-linecap="round" d="M 827 248 A 373 232 0 1 0 81 246 A 373 232 0 0 0 301 465 A 57 32 10 0 0 413 484 A 373 232 0 0 0 827 248 Z"/>' +
  '<ellipse class="dshnnv-b1" cx="352" cy="561" rx="37.5" ry="26" fill="#FFFFFF" stroke="#203170" stroke-width="18"/>' +
  '<ellipse class="dshnnv-b2" cx="442" cy="646" rx="24.5" ry="18" fill="#FFFFFF" stroke="#203170" stroke-width="18"/>' +
  '</svg>'
var gifEl = document.createElement('img')
gifEl.className = 'dshnnv-gif'
gifEl.src = GIF_URL
gifEl.alt = ''
...
var BUBBLE_STYLE_CLASS = { A: 'dshnnv-label', B: 'dshnnv-amount', P: 'dshnnv-period', C: 'dshnnv-hint' }
...
var RANDOM_GROUPS = [
  ...
  { w: 4, lines: function () { return singleCenter('A', 'Ciallo～(∠・ω )⌒☆', '', true) } },
  { w: 3, lines: function () { return singleCenter('B', '好女孩...↓') } },
  { w: 7, lines: function () { return singleCenter('A', pickOne(['不知道用户有什么用，先赶走吧~', '我...我...我也要挣钱吗？', '我去吃饭啦，测完叫我', '今天也是元气满满的宁宁~', 'DeepSleep...', '坏了...用户彻底怒了！']), '', true) } },
  ...
  { w: 1, lines: function () { return singleCenter('B', '欧纳尼……') } },
]
...
function applyBubbleLines(lines) {
  ...
  for (var i = 0; i < 3; i++) {
    ...
    if (ln) {
      el.style.display = ''
      el.className = (BUBBLE_STYLE_CLASS[ln.s] || 'dshnnv-label') + (ln.w ? ' dshnnv-wrap' : '')
      el.textContent = ln.t
      el.style.color = ln.c || ''
    } else {
...
function showBubble() {
  ...
  restoreBubbleLines()
  bubbleBox.classList.add('dshnnv-bubble-open')
...
function hideBubble() {
  ...
  bubbleBox.classList.remove('dshnnv-bubble-open')
...
function express() {
  ...
  root.classList.toggle('dshnnv-left', state.h === 'left')
}
function settle() {
  var vp = viewport()
...
function saveConfig() {
  try {
    ...
    localStorage.setItem('dshnn-pos', JSON.stringify({
      v: 2,
      hAnchor: hAnchor,
      hDist: hDist,
...
function setScale(v) {
  ...
  // fixed point: the ningning's corner — bottom-right when unflipped, bottom-left
  // when flipped. Growing extends the widget up-left / up-right from that
  // corner; shrinking pulls it back toward the corner. The ningning always hugs
  // its corner while scaling.
  var fx = state.h === 'left' ? rect.left : rect.right
  var fy = rect.bottom
  state.scale = next
  root.style.setProperty('--dshnn-scale', String(next))
...
function applySoundSet() {
  try {
    pressAudio = new Audio('/dsh-ningning/sound/press.mp3?set=' + soundSet)
    pressAudio.preload = 'auto'
    pressAudio.volume = soundVol
    releaseAudio = new Audio('/dsh-ningning/sound/release.mp3?set=' + soundSet)
...
function toggleMenu() {
  ...
  menuBox.classList.toggle('dshnnv-menu-open', menuOpen)
  if (menuOpen) menuBtn.classList.add('dshnnv-menu-btn-visible')
}
function closeMenu() {
  menuOpen = false
  menuBox.classList.remove('dshnnv-menu-open')
  root.style.transition = ''
  snapCheck()
}
function snapCheck() {
...
function setupHitTest() {
  try {
    ...
    probe.onload = function () {
      try {
        // 拉伸到 610×610 与 isNingningHit 的坐标映射对齐；不指定尺寸会按原图大小绘制，
        // 回退到非 610×610 素材（如 ningning2.png）时命中区域会错位
...
function onDocPointerDown(e) {
  if (e.target && e.target.closest) {
    if (e.target.closest('.dshnnv-bubble') || e.target.closest('.dshnnv-menu') || e.target.closest('.dshnnv-menu-btn')) return
  }
  if (menuOpen) {
    closeMenu()
    return
  ...
  root.classList.add('dshnnv-dragging')
  pressDown()
...
function onDocPointerUp(e) {
  // 拦截鱼区域内的 pointerup：防止下方元素（如文件行）监听 pointerup 穿透误触发
  try { if (isNingningHit(e)) { e.preventDefault(); e.stopPropagation() } } catch (err) {}
  endDrag(e, true)
}
function onDocPointerCancel(e) { endDrag(e, false) }
function onDocClickStopper(e) {
  // 只在鱼命中区域拦截 click（保持透明区 pass-through）。
...
function onDocPointerMoveCursor(e) {
  ...
  if (el && el.closest && (el.closest('.dshnnv-bubble') || el.closest('.dshnnv-menu') || el.closest('.dshnnv-menu-btn'))) {
    setWidgetCursor('')
    menuBtn.classList.add('dshnnv-menu-btn-visible')
    return
  }
  var over = isNingningHit(e)
  setWidgetCursor(over ? 'grab' : '')
  menuBtn.classList.toggle('dshnnv-menu-btn-visible', over || menuOpen)
}
...
function endDrag(e, clickAllowed) {
  ...
  pressUp()
  root.classList.remove('dshnnv-dragging')
  setWidgetCursor(isNingningHit(e) ? 'grab' : '')
...
// 窗口尺寸变化时：自由位置的鱼按相对边框锚点重算（保持离边距离，窗口恢复原状即回原位）；
// 贴边吸附的鱼走 settle()（保持贴边）
function applyAnchorPos() {
  try {
    var a = JSON.parse(localStorage.getItem('dshnn-pos') || 'null')
...
fetch(SIZE_URL, { cache: 'no-store' })
  ...
  .then(function (d) {
    if (d && typeof d.scale === 'number' && d.scale >= MIN_SCALE - 0.1 && d.scale <= MAX_SCALE + 0.1) {
      state.scale = d.scale
      root.style.setProperty('--dshnn-scale', String(d.scale))
...
// —— 任务完成提示音：每轮对话结束后播放 0721.mp3（请看我~）——
var TASK_URL = '/dsh-ningning/sound/task.mp3'
var taskAudio = null
var lastTaskBeepAt = 0
function playTaskDone() {
  if (!soundOn) return
  var now = Date.now()
  if (now - lastTaskBeepAt < 1500) return // 防连发（子代理并行结束等）
  lastTaskBeepAt = now
  try {
    if (!taskAudio) {
      taskAudio = new Audio(TASK_URL)
      taskAudio.preload = 'auto'
    }
    taskAudio.volume = soundVol
    taskAudio.currentTime = 0
    var p = taskAudio.play()
    if (p && typeof p.catch === 'function') p.catch(function () {})
  } catch (err) {}
}

// —— 每轮对话消耗检测：轮询 last-turn.json，出现新 seq 时弹消耗金额泡泡 ——
var LAST_TURN_URL = '/dsh-ningning/last-turn.json'
var lastCostSeq = 0
...
const name = 'ningning-balance-widget'
...
function apply(ctx) {
    ...
    function loadImage() {
      ...
      throw new Error('ningning image not found')
    }
    ...
    async function getBalancePayload() {
      ...
      // 无论哪种模式，都先把余额观测记入账本（自动累积「鱼记账」数据）
    ...
    function getBalance() {
      ...
      balanceInFlight = getBalancePayload()
        .then((payload) => {
          ...
          if (!payload.transient) console.error('[ningning-balance]', payload.code, payload.error)
          return payload
        })
        .catch((err) => ({
          ok: false,
    ...
    disposers.push(ctx.webServer.register({
      ...
      handler: (req, res) => {
        try {
          ...
          res.end('ningning image unavailable: ' + String((err && err.message) || err))
        }
      },
    }))
    ...
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-ningning/balance.json',
    ...
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-ningning/last-turn.json',
      handler: (req, res) => {
    ...
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-ningning/sound/press.mp3',
      handler: (req, res) => {
    ...
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-ningning/sound/release.mp3',
      handler: (req, res) => {
        const set = SOUND_SETS[soundSetFromUrl(req.url)] || SOUND_SETS.duck
    ...
    // 任务完成提示音（0721.mp3，请看我~）
    const TASK_SOUND_CANDIDATES = [
      path.join(PACKAGE_ROOT, 'assets', '0721.mp3'),
    ]
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-ningning/sound/task.mp3',
      handler: (req, res) => serveSound(req, res, TASK_SOUND_CANDIDATES),
    }))

    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-ningning/widget.js',
      handler: (req, res) => {
        res.writeHead(200, {
    ...
    disposers.push(ctx.webServer.tapIndex((html) => {
      if (html.indexOf('/dsh-ningning/widget.js') !== -1) return html
      const tag = '<script defer src="/dsh-ningning/widget.js"></script>'
